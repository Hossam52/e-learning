...
class StudentTestAnswersScreen extends StatelessWidget {
  StudentTestAnswersScreen({Key? key, required this.cubit, required this.test}) : super(key: key);
                              ...
                              children: [
                                Expanded(
                                  child: ListView.separated(
                                    itemCount: cubit.wrongQuestionsIndex.length,
                                    padding: EdgeInsets.symmetric(vertical: 35.h),
                                    separatorBuilder: (context, index) => SizedBox(height: 10.h,),
                                    itemBuilder: (context, index) {
                                      int questionIndex = cubit.wrongQuestionsIndex[index];
                                      var question = test.questions![questionIndex];
                                      return StudentAnswersBuildItem(
                                        deviceInfo: deviceInfo,
                                        question: question.questionText!,
                                        wrongAnswer: addAnswer(questionIndex, question, true),
                                        rightAnswer: addAnswer(questionIndex, question, false),
                                      );
                                    },
                                  ),
                                ),
  ...
  String addAnswer(int currentIndex, Question question, bool isWrong) {
    String? answer = isWrong ? cubit.studentChooseAnswerList[currentIndex] : question.answer;
    if(answer == '0') return question.chose1Text!;
    else if (answer == '1') return question.chose2Text!;
    else if (answer == '2') return question.chose3Text!;
    else return question.chose4Text!;
  }
}
